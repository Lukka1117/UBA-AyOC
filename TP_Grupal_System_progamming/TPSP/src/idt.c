...
/* DESCOMENTAR Y COMPLETAR: Dado un numero de de interrupcion asigna a `idt` la
 * entrada correspondiente con nivel 0 */
#define IDT_ENTRY0(numero)                                                 \
  idt[numero] = (idt_entry_t){.offset_31_16 = HIGH_16_BITS(&_isr##numero), \
                              .offset_15_0 = LOW_16_BITS(&_isr##numero),   \
                              .segsel = GDT_CODE_0_SEL,                    \
                              .type = INTERRUPT_GATE_TYPE,                 \
                              .dpl = PRIVILEGE_LEVEL_KERNEL,               \
                              .present = 0x1}

/* DESCOMENTAR Y COMPLETAR: Dado un numero de de interrupcion asigna a `idt` la
 * entrada correspondiente con nivel 3 */
#define IDT_ENTRY3(numero)                                                 \
  idt[numero] = (idt_entry_t){.offset_31_16 = HIGH_16_BITS(&_isr##numero), \
                              .offset_15_0 = LOW_16_BITS(&_isr##numero),   \
                              .segsel = GDT_CODE_0_SEL,                    \
                              .type = INTERRUPT_GATE_TYPE,                 \
                              .dpl = PRIVILEGE_LEVEL_USER,                 \
                              .present = 0x1}
...
void kernel_exception(control_regs cregs, segment_regs sregs,
                      general_regs gregs, kernel_error_frame frame)
{
  // breakpoint();
  ...
  print_hex(*((uint32_t *)gregs.esp), 8, STACK_START_COL, 21,
            C_BG_BLACK | C_FG_LIGHT_GREEN);
  print_hex(*((uint32_t *)gregs.esp + 1), 8, STACK_START_COL, 23,
            C_BG_BLACK | C_FG_LIGHT_GREEN);
  print_hex(*((uint32_t *)gregs.esp + 2), 8, STACK_START_COL, 25,
            C_BG_BLACK | C_FG_LIGHT_GREEN);
}
